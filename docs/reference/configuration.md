...
| `view`                         | string  | `list`                        | Layout the card renders in — `list` stacks each day above the next, `column` places days side by side. See [Column View](/features/column-view)                                                                                                                                                                                                                                                                                                     |
...
| `show_empty_days`              | boolean | `false`                       | Whether to show days with no events (with "No events" message). Column view defaults this to `true` and does not inherit the top-level value — see [Options That Start From a Different Default](/features/column-view#options-that-start-from-a-different-default)                                                                                                                                                                                 |
...
| `show_countdown`                | boolean           | `false`                                            | Show clock units for starts later today, "tomorrow" for the next date, and natural day/month/year wording for later dates, measured between local calendar dates                                                 |
...
| `progress_bar_width`            | string            | _per placement_                                    | Width of the progress bar. Unset, each layout uses its own: `60px` on the time row in list view, `80%` of the column in column view, where the bar takes a row of its own. A value here replaces both            |
